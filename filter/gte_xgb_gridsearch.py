...
init_env(name='flt_gte_xgb', cuda_devices=2)
...
df_commits = pd.read_parquet(settings.DATASETS_PATH / 'dataset_flt.parquet')
df_repos = pd.read_csv(settings.DATASETS_PATH / 'repos_flt.csv', on_bad_lines='skip')
...
df_ds = pd.concat([df_commits, df_embs], axis=1)\
    .drop(columns=['nameWithOwner', 'sha', 'authorName', 'authorEmail', 'date', 'releaseTag', 'body', 'type'])
...
ds_train, ds_test = train_test_split(
    df_ds, test_size=0.3, random_state=settings.seed, stratify=df_ds['isInRN']
)
ds_test, ds_valid = train_test_split(
    ds_test, test_size=0.33, random_state=settings.seed, stratify=ds_test['isInRN']
)
...
X_train, y_train = ds_train.drop(columns=['isInRN']), ds_train['isInRN']
X_test, y_test = ds_test.drop(columns=['isInRN']), ds_test['isInRN']
X_valid, y_valid = ds_valid.drop(columns=['isInRN']), ds_valid['isInRN']
